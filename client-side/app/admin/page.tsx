...
export default function AdminPage() {
    ...
    const handleOrderStatus = async (id: number, status: string) => {
        await fetch(`http://localhost:8081/api/orders/${id}/status?status=${status}`, {
            method: 'PUT'
        });
        loadAll();
    };
    ...
    const handleResStatus = async (id: number, status: string) => {
        await fetch(`http://localhost:8081/api/reservations/${id}/status?status=${status}`, {
            method: 'PUT'
        });
        loadAll();
    };
    ...
    const handleDeleteItem = async (id: number) => {
        if (confirm('Delete this item?')) {
            await fetch(`http://localhost:8081/api/menu/${id}`, { method: 'DELETE' });
            loadAll();
        }
    };
    ...
    if (!user) return (
        <div style={{
            ...
            <div style={{ color: '#C9A84C', fontSize: 18, fontFamily: 'Playfair Display, serif' }}>
                Loading Dashboard...
            </div>
            <div style={{ color: '#9A9080', fontSize: 14 }}>Please wait...</div>
        </div>
    );
        ...
        <main style={{ background: '#0D0D0D', minHeight: '100vh', paddingTop: 80 }}>
            ...
            <nav style={{
                ...
                <Link href="/" style={{
                    ...
                    fontWeight: 900, color: '#C9A84C', textDecoration: 'none'
                }}>
          ...
          <span style={{ color: '#9A9080', fontSize: 14 }}>
            ...
            <section style={{ padding: '40px 60px 80px' }}>
                    ...
                    {[
                        ...
                        <div key={stat.label} style={{
                            ...
                            <div>
                                ...
                                <div style={{ color: stat.color, fontSize: 36, fontFamily: 'Playfair Display, serif', fontWeight: 900 }}>
                                    {stat.value}
                                </div>
                            </div>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 32 }}>
                    {tabs.map(t => (
                        <button key={t.key}
                                onClick={() => setTab(t.key as 'orders' | 'reservations' | 'menu')}
                                style={{
                ...
                {tab === 'orders' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        {orders.length === 0 ? (
                            <div style={{
                                textAlign: 'center', color: '#9A9080',
                                padding: 60, background: '#1A1A1A', borderRadius: 12
                            }}>
                      ...
                      {order.status}
                                    ...
                                    <div style={{
                                        background: '#252525', borderRadius: 8,
                                        padding: '12px 16px', marginBottom: 16
                                    }}>
                                        <div style={{ color: '#C9A84C', fontSize: 12, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>
                                            Order Items
                                        </div>
                                        <div style={{ color: '#E8E0D0', fontSize: 14 }}>{order.items}</div>
                                        {order.notes && (
                                            <div style={{ color: '#9A9080', fontSize: 13, marginTop: 6 }}>📝 {order.notes}</div>
                                        )}
                                    </div>
                                ...
                                <div key={res.id} style={{
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                                        <div>
                                            ...
                                            {res.notes && (
                                                <div style={{ color: '#9A9080', fontSize: 13, marginTop: 4 }}>📝 {res.notes}</div>
                                            )}
                                        </div>
                                        <span style={{
            ...
            <footer style={{
                ...
                <p style={{ color: '#9A9080', fontSize: 14 }}>
                    © 2026 Dinu&apos;s Tasty Admin Panel | Developed by Paramith Kavisha
                </p>
            </footer>
